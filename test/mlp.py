...
class MLPTestCase(unittest.TestCase):
    ...
    def test_one_hot(self):
        pipeline = Pipeline(DataLoader(text_processor=self.text_processor, n_grams=3),
                            TextEncoder(encode_type=MLPTestCase.EncodingAliases.ONE_HOT),
                            MLPClassifier(num_labels=3))
        pipeline(train=MLPTestCase.INPUT_FILE, test=MLPTestCase.OUTPUT_FILE)
        pipeline.save(MLPTestCase.SAVED_PATH)

        _pipeline = Pipeline.load(MLPTestCase.SAVED_PATH)
    ...
    def test_bow(self):
        pipeline = Pipeline(DataLoader(text_processor=self.text_processor, n_grams=3),
                            TextEncoder(encode_type=MLPTestCase.EncodingAliases.BOW),
                            MLPClassifier(num_labels=3))
        pipeline(train=MLPTestCase.INPUT_FILE, test=MLPTestCase.OUTPUT_FILE)
        pipeline.save(MLPTestCase.SAVED_PATH)
    ...
    def test_tf_idf(self):
        pipeline = Pipeline(DataLoader(text_processor=self.text_processor, n_grams=3),
                            TextEncoder(encode_type=MLPTestCase.EncodingAliases.TF_IDF),
                            MLPClassifier(num_labels=3))
        pipeline(train=MLPTestCase.INPUT_FILE, test=MLPTestCase.OUTPUT_FILE)
        pipeline.save(MLPTestCase.SAVED_PATH)
    ...
    def test_word2vec(self):
        pipeline = Pipeline(DataLoader(text_processor=self.text_processor, n_grams=3),
                            TextEncoder(encode_type=MLPTestCase.EncodingAliases.W2V,
                                        model_path=MLPTestCase.W2V_PRETRAINED),
                            MLPClassifier(num_labels=3))
        pipeline(train=MLPTestCase.INPUT_FILE, test=MLPTestCase.OUTPUT_FILE)
        pipeline.save(MLPTestCase.SAVED_PATH)
