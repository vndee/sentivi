...
class NearestCentroidTestCase(unittest.TestCase):
    ...
    def test_one_hot(self):
        pipeline = Pipeline(DataLoader(text_processor=self.text_processor, n_grams=3),
                            TextEncoder(encode_type=NearestCentroidTestCase.EncodingAliases.ONE_HOT),
                            NearestCentroidClassifier(num_labels=3))
        pipeline(train=NearestCentroidTestCase.INPUT_FILE, test=NearestCentroidTestCase.OUTPUT_FILE)
        pipeline.save(NearestCentroidTestCase.SAVED_PATH)

        _pipeline = Pipeline.load(NearestCentroidTestCase.SAVED_PATH)
    ...
    def test_bow(self):
        pipeline = Pipeline(DataLoader(text_processor=self.text_processor, n_grams=3),
                            TextEncoder(encode_type=NearestCentroidTestCase.EncodingAliases.BOW),
                            NearestCentroidClassifier(num_labels=3))
        pipeline(train=NearestCentroidTestCase.INPUT_FILE, test=NearestCentroidTestCase.OUTPUT_FILE)
        pipeline.save(NearestCentroidTestCase.SAVED_PATH)
    ...
    def test_tf_idf(self):
        pipeline = Pipeline(DataLoader(text_processor=self.text_processor, n_grams=3),
                            TextEncoder(encode_type=NearestCentroidTestCase.EncodingAliases.TF_IDF),
                            NearestCentroidClassifier(num_labels=3))
        pipeline(train=NearestCentroidTestCase.INPUT_FILE, test=NearestCentroidTestCase.OUTPUT_FILE)
        pipeline.save(NearestCentroidTestCase.SAVED_PATH)
    ...
    def test_word2vec(self):
        pipeline = Pipeline(DataLoader(text_processor=self.text_processor, n_grams=3),
                            TextEncoder(encode_type=NearestCentroidTestCase.EncodingAliases.W2V,
                                        model_path=NearestCentroidTestCase.W2V_PRETRAINED),
                            NearestCentroidClassifier(num_labels=3))
        pipeline(train=NearestCentroidTestCase.INPUT_FILE, test=NearestCentroidTestCase.OUTPUT_FILE)
        pipeline.save(NearestCentroidTestCase.SAVED_PATH)
