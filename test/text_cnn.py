...
from sentivi.classifier import TextCNNClassifier


class TextCNNTestCase(unittest.TestCase):
    ...
    def test_one_hot(self):
        pipeline = Pipeline(DataLoader(text_processor=self.text_processor, n_grams=3),
                            TextEncoder(encode_type=TextCNNTestCase.EncodingAliases.ONE_HOT),
                            TextCNNClassifier(num_labels=3))
        pipeline(train=TextCNNTestCase.INPUT_FILE, test=TextCNNTestCase.OUTPUT_FILE)
        pipeline.save(TextCNNTestCase.SAVED_PATH)

        _pipeline = Pipeline.load(TextCNNTestCase.SAVED_PATH)
    ...
    def test_bow(self):
        pipeline = Pipeline(DataLoader(text_processor=self.text_processor, n_grams=3),
                            TextEncoder(encode_type=TextCNNTestCase.EncodingAliases.BOW),
                            TextCNNClassifier(num_labels=3))
        pipeline(train=TextCNNTestCase.INPUT_FILE, test=TextCNNTestCase.OUTPUT_FILE)
        pipeline.save(TextCNNTestCase.SAVED_PATH)
    ...
    def test_tf_idf(self):
        pipeline = Pipeline(DataLoader(text_processor=self.text_processor, n_grams=3),
                            TextEncoder(encode_type=TextCNNTestCase.EncodingAliases.TF_IDF),
                            TextCNNClassifier(num_labels=3))
        pipeline(train=TextCNNTestCase.INPUT_FILE, test=TextCNNTestCase.OUTPUT_FILE)
        pipeline.save(TextCNNTestCase.SAVED_PATH)
    ...
    def test_word2vec(self):
        pipeline = Pipeline(DataLoader(text_processor=self.text_processor, n_grams=3),
                            TextEncoder(encode_type=TextCNNTestCase.EncodingAliases.W2V,
                                        model_path=TextCNNTestCase.W2V_PRETRAINED),
                            TextCNNClassifier(num_labels=3))
        pipeline(train=TextCNNTestCase.INPUT_FILE, test=TextCNNTestCase.OUTPUT_FILE)
        pipeline.save(TextCNNTestCase.SAVED_PATH)
