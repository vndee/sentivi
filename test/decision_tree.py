...
class DecisionTreeTestCase(unittest.TestCase):
    ...
    def test_one_hot(self):
        pipeline = Pipeline(DataLoader(text_processor=self.text_processor, n_grams=3),
                            TextEncoder(encode_type=DecisionTreeTestCase.EncodingAliases.ONE_HOT),
                            DecisionTreeClassifier(num_labels=3))
        pipeline(train=DecisionTreeTestCase.INPUT_FILE, test=DecisionTreeTestCase.OUTPUT_FILE)
        pipeline.save(DecisionTreeTestCase.SAVED_PATH)

        _pipeline = Pipeline.load(DecisionTreeTestCase.SAVED_PATH)
    ...
    def test_bow(self):
        pipeline = Pipeline(DataLoader(text_processor=self.text_processor, n_grams=3),
                            TextEncoder(encode_type=DecisionTreeTestCase.EncodingAliases.BOW),
                            DecisionTreeClassifier(num_labels=3))
        pipeline(train=DecisionTreeTestCase.INPUT_FILE, test=DecisionTreeTestCase.OUTPUT_FILE)
        pipeline.save(DecisionTreeTestCase.SAVED_PATH)
    ...
    def test_tf_idf(self):
        pipeline = Pipeline(DataLoader(text_processor=self.text_processor, n_grams=3),
                            TextEncoder(encode_type=DecisionTreeTestCase.EncodingAliases.TF_IDF),
                            DecisionTreeClassifier(num_labels=3))
        pipeline(train=DecisionTreeTestCase.INPUT_FILE, test=DecisionTreeTestCase.OUTPUT_FILE)
        pipeline.save(DecisionTreeTestCase.SAVED_PATH)
    ...
    def test_word2vec(self):
        pipeline = Pipeline(DataLoader(text_processor=self.text_processor, n_grams=3),
                            TextEncoder(encode_type=DecisionTreeTestCase.EncodingAliases.W2V,
                                        model_path=DecisionTreeTestCase.W2V_PRETRAINED),
                            DecisionTreeClassifier(num_labels=3))
        pipeline(train=DecisionTreeTestCase.INPUT_FILE, test=DecisionTreeTestCase.OUTPUT_FILE)
        pipeline.save(DecisionTreeTestCase.SAVED_PATH)
