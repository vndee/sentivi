...
from sentivi.classifier import SGDClassifier
...
class SGDTestCase(unittest.TestCase):
    ...
    def test_one_hot(self):
        pipeline = Pipeline(DataLoader(text_processor=self.text_processor, n_grams=3),
                            TextEncoder(encode_type=SGDTestCase.EncodingAliases.ONE_HOT),
    ...
    def test_bow(self):
        pipeline = Pipeline(DataLoader(text_processor=self.text_processor, n_grams=3),
                            TextEncoder(encode_type=SGDTestCase.EncodingAliases.BOW),
                            SGDClassifier(num_labels=3))
        pipeline(train=SGDTestCase.INPUT_FILE, test=SGDTestCase.OUTPUT_FILE)
        pipeline.save(SGDTestCase.SAVED_PATH)
    ...
    def test_word2vec(self):
        pipeline = Pipeline(DataLoader(text_processor=self.text_processor, n_grams=3),
                            TextEncoder(encode_type=SGDTestCase.EncodingAliases.W2V,
                                        model_path=SGDTestCase.W2V_PRETRAINED),
                            SGDClassifier(num_labels=3))
        pipeline(train=SGDTestCase.INPUT_FILE, test=SGDTestCase.OUTPUT_FILE)
        pipeline.save(SGDTestCase.SAVED_PATH)
